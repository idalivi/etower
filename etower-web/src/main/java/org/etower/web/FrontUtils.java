...
import static org.etower.Constants.TPLDIR_COMMON;
import static org.etower.Constants.TPLDIR_STYLE_LIST;
import static org.etower.Constants.TPLDIR_TAG;
import static org.etower.Constants.TPL_STYLE_PAGE_CHANNEL;
import static org.etower.Constants.TPL_SUFFIX;
import static org.etower.common.web.Constants.MESSAGE;
import static org.etower.common.web.Constants.UTF8;
import static org.etower.common.web.ProcessTimeFilter.START_TIME;
import static org.etower.common.web.freemarker.DirectiveUtils.PARAM_TPL_SUB;
...
import org.etower.common.web.freemarker.DirectiveUtils;
...
public class FrontUtils {
	...
	public static String showLogin(HttpServletRequest request,
			Map<String, Object> model, EtSite site, String message) {
		if (!StringUtils.isBlank(message)) {
			model.put(MESSAGE, message);
		}
		StringBuilder buff = new StringBuilder("redirect:");
		buff.append(site.getEtConfig().getLoginUrl()).append("?");
		buff.append(RETURN_URL).append("=");
		buff.append(RequestUtils.getLocation(request));
		if (!StringUtils.isBlank(site.getEtConfig().getProcessUrl())) {
			buff.append("&").append(PROCESS_URL).append(site.getEtConfig().getProcessUrl());
		}
	 ...
	 */
	public static String showLogin(HttpServletRequest request,
			Map<String, Object> model, EtSite site) {
	...
	public static void frontData(HttpServletRequest request,
			Map<String, Object> map, EtSite site) {
	...
	public static void frontData(Map<String, Object> map, EtSite site,
			EtUser user, String location, Long startTime) {
		if (startTime != null) {
		...
		map.put(SITE, site);
		String ctx = site.getEtConfig().getContextPath() == null ? "" : site.getEtConfig().getContextPath();
		map.put(BASE, ctx);
	 ...
	 */
	public static EtSite getSite(Environment env)
			throws TemplateModelException {
		TemplateModel model = env.getGlobalVariable(SITE);
		if (model instanceof AdapterTemplateModel) {
			return (EtSite) ((AdapterTemplateModel) model)
					.getAdaptedObject(EtSite.class);
		} else {
	...
	public static void includePagination(EtSite site,
	 ...
	 * @throws TemplateException
	 */
	public static void includeTpl(String tplName, EtSite site,
			Map<String, TemplateModel> params, Environment env)
	 ...
	 * @throws TemplateException
	 */
	public static void includeTpl(String listStyle, EtSite site,
