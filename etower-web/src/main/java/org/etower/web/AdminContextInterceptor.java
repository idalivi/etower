...
public class AdminContextInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		// 获得站点
		EtSite site = getSite(request, response);
		SysUtils.setSite(request, site);
		// Site加入线程变量
		SysThreadVariable.setSite(site);

		// 获得用户
		EtUser user = null;
		if (adminId != null) {
			// 指定管理员（开发状态）
			user = etUserMng.findById(adminId);
			if (user == null) {
		...
		// 不属于该站点的管理员，提示无权限。
//		if (!user.getEtRoles().contains(site)) {
//			request.setAttribute(MESSAGE,
//					MessageResolver.getMessage(request, "login.notInSite"));
//			response.sendError(HttpServletResponse.SC_FORBIDDEN);
//			return false;
//		}
//		boolean viewonly = user.getIsViewonlyAdmin();
//		// 没有访问权限，提示无权限。
...
//		}
	...
	public void postHandle(HttpServletRequest request,
		...
		if (auth && user != null && !user.getEtRoles().isEmpty() && mav != null
				&& mav.getModelMap() != null && mav.getViewName() != null
				&& !mav.getViewName().startsWith("redirect:")) {
			mav.getModelMap().addAttribute(PERMISSION_MODEL, set); //????? user.getPerms
		}
	}
	...
	public void afterCompletion(HttpServletRequest request,
		...
		SysThreadVariable.removeUser();
		SysThreadVariable.removeSite();
	}
	 ...
	 */
	private EtSite getSite(HttpServletRequest request,
			HttpServletResponse response) {
		EtSite site = getByParams(request, response);
		if (site == null) {
	...
	private EtSite getByParams(HttpServletRequest request,
			HttpServletResponse response) {
		String p = request.getParameter(SITE_PARAM);
		if (!StringUtils.isBlank(p)) {
			try {
				Long siteId = Long.parseLong(p);
				EtSite site = etSiteMng.findById(siteId);
				if (site != null) {
	...
	private EtSite getByCookie(HttpServletRequest request) {
		Cookie cookie = CookieUtils.getCookie(request, SITE_COOKIE);
		if (cookie != null) {
			String v = cookie.getValue();
			if (!StringUtils.isBlank(v)) {
				try {
					Long siteId = Long.parseLong(v);
					return etSiteMng.findById(siteId);
				} catch (NumberFormatException e) {
	...
	private EtSite getByDomain(HttpServletRequest request) {
		String domain = request.getServerName();
		if (!StringUtils.isBlank(domain)) {
			return etSiteMng.findByDomain(domain);
		}
		return null;
	}

	private EtSite getByDefault() {
	...
	private boolean permistionPass(String uri, List<String> perms,
			boolean viewOnly) {
		String u = null;
		int i;
	...
	private static String getURI(HttpServletRequest request)
		...
		if (start <= 0) {
			throw new IllegalStateException(
					"admin access path not like '/etadmin/etower/...' pattern: "
							+ uri);
		}
	...
	@Autowired
	public void setEtSiteMng(EtSiteMng etSiteMng) {
		this.etSiteMng = etSiteMng;
	}

	@Autowired
	public void setEtUserMng(EtUserMng etUserMng) {
		this.etUserMng = etUserMng;
	}

	@Autowired
	public void setAuthenticationMng(AuthenticationMng authenticationMng) {
		this.authenticationMng = authenticationMng;
	}
